...
func FeiShuCallBackConsumer() {

	log.Infof("mq消息-飞书回调消费者启动成功")

	fsCallbackInfo := config.GetMqFeiShuCallBackTopicConfig()
	...
	_ = client.ConsumeMessage(fsCallbackInfo.Topic, fsCallbackInfo.GroupId, func(message *model.MqMessageExt) errors.SystemErrorInfo {
		...
		log.Infof("mq消息-飞书回调消费信息 topic %s, value %s", message.Topic, message.Body)
		...
		//msgTime := fsCallBackBo.MsgTime
		//if ! msgTime.IsZero(){
		//	//判断消息的时效性, 十分钟认为消息无效
		//	if time.Now().Sub(msgTime) > 10 * time.Minute{
		//		log.Error("飞书回调消息超出时效性")
		//		return nil
		//	}
		//}
		...
		msgId := uuid.NewUuid()
		log.Infof("飞书回调消息异步处理准备msgId:%s, msgBody: %s", msgId, message.Body)
		_, handleErr := handler.Handle(fsCallBackBo.Data)
		if handleErr != nil {
			log.Infof("飞书回调消息异步处理失败msgId:%s, msgBody: %s", msgId, message.Body)
			log.Error(handleErr)
			return handleErr
		}
