...
func FeiShuCallBackContactScopeChangeConsumer() {

	log.Infof("mq消息-飞书回调通讯录授权范围消费者启动成功")

	fsCallbackInfo := config.GetFeishuCallBackContactScopeChange()
	...
	_ = client.ConsumeMessage(fsCallbackInfo.Topic, fsCallbackInfo.GroupId, func(message *model.MqMessageExt) errors.SystemErrorInfo {
		...
		log.Infof("mq消息-飞书回调通讯录授权范围信息 topic %s, value %s", message.Topic, message.Body)
		...
		msgId := uuid.NewUuid()
		log.Infof("飞书回调通讯录授权范围异步处理准备msgId:%s, msgBody: %s", msgId, message.Body)
		_, handleErr := handler.Handle(fsCallBackBo.Data)
		if handleErr != nil {
			log.Infof("飞书回调通讯录授权范围异步处理失败msgId:%s, msgBody: %s", msgId, message.Body)
			log.Error(handleErr)
			return handleErr
		}
