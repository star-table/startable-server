...
func getMD5FromNewAuthString(r *http.Request) (string, []byte, error) {
	...
	if errUnescape != nil {
		...
		return strCallbackBody, byteMD5, errUnescape
	}
	...
	return strCallbackBody, byteMD5, nil
}
...
func responseSuccess(w http.ResponseWriter, strResponseBody string) {
...
func responseFailed(w http.ResponseWriter, strResponseBody string) {
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Content-Length", strconv.Itoa(len(strResponseBody)))
	w.WriteHeader(http.StatusOK)
	w.Write([]byte(strResponseBody))
	fmt.Printf("\nPost Response : 400 BAD %s. \n", strResponseBody)
}
