...
func UploadOssByFsImageKey(orgId int64, imageKey string, isApp bool) (string, errs.SystemErrorInfo) {
	...
	imageInfo, imageErr := tenant.GetImage(imageKey, isApp)
	if imageErr != nil {
		log.Error(imageErr)
	...
	//公网地址
	//ossTrulyConfig := "https://oss-cn-shanghai.aliyuncs.com"
	//内网地址
	ossTrulyConfig := "https://oss-cn-shanghai-internal.aliyuncs.com"

	client, clientErr := oss.New(ossTrulyConfig, ossConfig.AccessKeyId, ossConfig.AccessKeySecret)
	if clientErr != nil {
		log.Error(clientErr)
		return "", errs.OssError
	}

	// 获取存储空间。
	bucket, bucketErr := client.Bucket("polaris-hd2")
	if bucketErr != nil {
		log.Error(bucketErr)
		return "", nil
	}
	...
	//		} else {
	//			return "", errs.FeishuImageIsEmpty
	//		}
	//	}
	//	i++
	//
	...
	nextPos, appendErr := bucket.AppendObject(objName, bytes.NewReader(buf[:n]), nextPos)
	if appendErr != nil {
		log.Error(appendErr)
		return "", errs.FeishuImageIsEmpty
	}
	...
	//剩下的一次性写入
	nextPos, appendErr = bucket.AppendObject(objName, imageInfo, nextPos)
	if appendErr != nil {
		log.Error(appendErr)
		return "", errs.FeishuImageIsEmpty
	}

	return ossConfig.EndPoint + "/" + objName, nil
}
